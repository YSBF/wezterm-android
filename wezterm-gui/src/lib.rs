//! The wezterm GUI.
//!
//! This is a library rather than a plain binary so that the same code can be
//! driven from two different entry points: `src/main.rs` for the desktop
//! binary, and a `cdylib` loaded by an Android Activity, which has no `main`
//! of its own. See `android.rs` for the latter.
